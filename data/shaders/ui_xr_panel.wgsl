...
fn fs_main(in: VertexOutput) -> FragmentOutput {
    ...
    var size : vec2f = ui_data.xr_info.xy;
    var position = ui_data.xr_info.zw;
    // center stuff
    position -= vec2f(0.5);

#ifdef ALBEDO_TEXTURE
    var corrected_uv : vec2f = (vec2f(in.uv.x, in.uv.y) % (position * 2.0)) / (size);
    // corrected_uv.x -= size.x;
    ...
    out.color = vec4f(mix(vec3f(corrected_uv, 0.0), final_color, 0.0), alpha);
