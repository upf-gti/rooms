...
@group(0) @binding(1) var<storage, read_write> aabb_group_storage : array<sPaddedAABB>;
...
fn compute(@builtin(workgroup_id) id: vec3<u32>, @builtin(local_invocation_index) local_id: u32)
{
    ...
    for (var i : u32 = 0u; i < 8u; i++) {
        local_AABB = merge_padded_aabb(local_AABB, aabb_group_storage[local_id * 8u + i]);
    }
    ...
    if (local_id == 0u) {
        ...
        gpu_return_results.sculpt_aabb_min = local_AABB.min;
        gpu_return_results.sculpt_aabb_max = local_AABB.max;
    }
}
