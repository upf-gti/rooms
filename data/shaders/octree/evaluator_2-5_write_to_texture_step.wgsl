...
struct sJobCounters {
    bricks_to_interval_eval_count : i32,
    bricks_to_write_to_tex_count : i32
};

@group(0) @binding(0) var<storage, read_write> num_brinks_by_workgroup : u32;
@group(0) @binding(2) var<storage, read> job_counters : sJobCounters;
...
fn compute() 
{
    ...
    let float_bricks_to_write_count : f32 = f32(job_counters.bricks_to_write_to_tex_count);
    num_brinks_by_workgroup = u32(ceil(float_bricks_to_write_count / 65534.0));
    dispatch_counter.wg_x = u32(min(float_bricks_to_write_count, 65534.0));
}