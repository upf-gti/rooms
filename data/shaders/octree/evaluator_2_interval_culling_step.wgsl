...
@group(0) @binding(0) var<storage, read_write> bricks_to_interval_eval_count : atomic<i32>;
...
fn brick_create_or_reevaluate(octree_index : u32, is_current_brick_filled : bool, is_interior_brick : bool, octant_center : vec3f) {
    let prev_counter : i32 = atomicAdd(&bricks_to_write_to_tex_count, 1);
    ...
    bricks_to_write_to_tex_buffer[prev_counter] = octree_index;
}
...
fn compute(@builtin(local_invocation_index) thread_id: u32, @builtin(num_workgroups) workgroup_size : vec3u) 
{
    ...
    if (thread_id == 0u) {
        ...
        let starting_brick_idx : i32 = atomicSub(&bricks_to_interval_eval_count, 512);
    ...
    if (thread_id < brick_to_eval_wg_size) { 
        let brick_id : u32 = bricks_to_eval_wg_buffer[thread_id];
