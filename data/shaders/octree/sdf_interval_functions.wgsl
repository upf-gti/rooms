...
fn eval_interval_stroke_sphere_smooth_union(position : mat3x3f, current_surface : vec2f, curr_stroke: ptr<storage, Stroke>, curr_edit_list : ptr<storage, array<Edit>>, dimension_margin : vec4f) -> vec2f
{
    ...
    let edit_start_and_count : vec2u = stroke_get_edit_data(curr_stroke);
    let parameters : vec4f = stroke_get_params(curr_stroke);;
    let smooth_factor : f32 = parameters.w;
    let cap_value : f32 = parameters.y;

    let ending_idx : u32 = edit_start_and_count.x + edit_start_and_count.y;

    if(cap_value > 0.0) {
        for(var i : u32 = edit_start_and_count.x; i < ending_idx; i++) {
...
fn eval_interval_stroke_sphere_smooth_substract(position : mat3x3f, current_surface : vec2f, curr_stroke: ptr<storage, Stroke>, curr_edit_list : ptr<storage, array<Edit>>, dimension_margin : vec4f) -> vec2f
{
    var result_surface : vec2f = current_surface;
    var tmp_surface : vec2f;
...
fn eval_interval_stroke_vesica_smooth_union(position : mat3x3f, current_surface : vec2f, curr_stroke: ptr<storage, Stroke>, curr_edit_list : ptr<storage, array<Edit>>, dimension_margin : vec4f) -> vec2f
{
    var result_surface : vec2f = current_surface;
    var tmp_surface : vec2f;
...
fn evaluate_stroke_interval( position: mat3x3f, stroke: ptr<storage, Stroke, read>, curr_edit_list : ptr<storage, array<Edit>, read>, current_surface : vec2f, center : vec3f, half_size : f32) -> vec2f
{
    ...
    let edit_count : u32 = stroke_get_edit_data(stroke).y;
    let parameters : vec4f = stroke_get_params(stroke);
    ...
    switch(curr_stroke_code) {
        ...
        case SD_SPHERE_SMOOTH_OP_SUBSTRACTION: {
            result_surface = eval_interval_stroke_sphere_smooth_substract(position, current_surface, stroke, curr_edit_list, vec4f(0.0));
            break;
        }
        case SD_BOX_SMOOTH_OP_UNION: {
            result_surface = eval_interval_stroke_box_smooth_union(position, current_surface, stroke, curr_edit_list, vec4f(0.0));
            break;
        }
        ...
        case SD_CAPSULE_SMOOTH_OP_UNION: {
            result_surface = eval_interval_stroke_capsule_smooth_union(position, current_surface, stroke, curr_edit_list, vec4f(0.0));
            break;
        }
        case SD_CAPSULE_SMOOTH_OP_SUBSTRACTION: {
            result_surface = eval_interval_stroke_capsule_smooth_union(position, initial_surface, stroke, curr_edit_list, vec4f(0.0));
            result_surface = opSmoothSubtractionInterval(current_surface, result_surface, smooth_factor);
            break;
        }
        case SD_CONE_SMOOTH_OP_UNION: {
            result_surface = eval_interval_stroke_cone_smooth_union(position, current_surface, stroke, curr_edit_list, vec4f(0.0));
            break;
        }
        case SD_CONE_SMOOTH_OP_SUBSTRACTION: {
            result_surface = eval_interval_stroke_cone_smooth_union(position, initial_surface, stroke, curr_edit_list, vec4f(0.0));
            result_surface = opSmoothSubtractionInterval(current_surface, result_surface, smooth_factor);
            break;
        }
        case SD_CYLINDER_SMOOTH_OP_UNION: {
            result_surface = eval_interval_stroke_cylinder_smooth_union(position, current_surface, stroke, curr_edit_list, vec4f(0.0));
            break;
        }
        case SD_CYLINDER_SMOOTH_OP_SUBSTRACTION: {
            result_surface = eval_interval_stroke_cylinder_smooth_union(position, initial_surface, stroke, curr_edit_list, vec4f(0.0));
            result_surface = opSmoothSubtractionInterval(current_surface, result_surface, smooth_factor);
            break;
        }
        case SD_TORUS_SMOOTH_OP_UNION: {
            result_surface = eval_interval_stroke_torus_smooth_union(position, current_surface, stroke, curr_edit_list, vec4f(0.0));
            break;
        }
        case SD_TORUS_SMOOTH_OP_SUBSTRACTION: {
            result_surface = eval_interval_stroke_torus_smooth_union(position, initial_surface, stroke, curr_edit_list, vec4f(0.0));
            result_surface = opSmoothSubtractionInterval(current_surface, result_surface, smooth_factor);
            break;
        }
        case SD_VESICA_SMOOTH_OP_UNION: {
            result_surface = eval_interval_stroke_vesica_smooth_union(position, current_surface, stroke, curr_edit_list, vec4f(0.0));
            break;
        }
        case SD_VESICA_SMOOTH_OP_SUBSTRACTION: {
            result_surface = eval_interval_stroke_vesica_smooth_union(position, initial_surface, stroke, curr_edit_list, vec4f(0.0));
            result_surface = opSmoothSubtractionInterval(current_surface, result_surface, smooth_factor);
            break;
        }
        default: {}
