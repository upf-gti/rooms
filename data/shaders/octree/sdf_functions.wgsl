...
fn evaluate_edit( position : vec3f, primitive : u32, operation : u32, parameters : vec4f, current_surface : Surface, stroke_material : Material, edit : Edit) -> Surface
{
    ...
    switch (primitive) {
        ...
        case SD_BEZIER: {
            ...
            pSurface = sdQuadraticBezier(position, edit.position, edit.position + vec3f(0.0, 0.5, 0.0), edit.position + vec3f(0.2, 0.1, 0.0), curve_thickness, edit.rotation, stroke_material);
            break;
        }
        default: {
            break;
